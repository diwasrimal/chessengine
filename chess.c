...
void findSquareName(int square, char *name);
...
void moveDecode(Move m, MoveFlag *flag, int *src_sq, int *dst_sq);
...
int main(int argc, char **argv)
{
    ...
    printBoard(b);
    MoveList move_list = generateMoves(b);
    ...
    for (size_t i = 0; i < move_list.count; i++) {
        ...
        moveDecode(m, &flag, &src_sq, &dst_sq);
        findSquareName(src_sq, src);
        findSquareName(dst_sq, dst);
        printf("%s%s[%04llu] ", src, dst, decToBin(flag));
    }
    printf("\n");

    src_sq = squareNameToIdx("b1");
    dst_sq = squareNameToIdx("c3");
...
void printBoard(const Board b)
{
    ...
    findSquareName(b.ep_square, epsquare_name);
...
MoveList generateMoves(const Board b)
{
    MoveList all_moves = {.moves = {EMPTY_MOVE}, .count = 0};
    ...
    for (int rank = 0; rank < 8; rank++) {
        for (int file = 0; file < 8; file++) {
            ...
            for (size_t i = 0; i < list.count; i++) {
                all_moves.moves[all_moves.count++] = list.moves[i];
            }
        }
    }
    return all_moves;
...
{
    *flag = (m & FLAG_MASK) >> 12;
    *src_sq = (m & SRC_SQ_MASK) >> 6;
    *dst_sq = m & DST_SQ_MASK;
}
